    ...
    setup(build2) {
      ...
      build2.onLoad({
        ...
        if (!imports.length && !exports.length) {
          ...
          proxyModule.push(`export { ${specifiers.join(",")} } from "${relativePath}"`, `export default require("${relativePath}")`);
        } else {
    ...
    async transform(code, id) {
      ...
      for (const importInfo of imports) {
        ...
        if (modSource.endsWith(".svg")) {
          const resolvedUrl = import_path6.default.join(import_path6.default.dirname(id), modSource);
  ...
  return {
    ...
    async resolveId(id, importer) {
      if (import_path7.default.isAbsolute(id)) {
        ...
        let resolvedId;
        if (hasExtension) {
          resolvedId = import_resolve2.default.sync(id, { basedir: import_path7.default.dirname(importer) });
